...
for i, study in enumerate(STUDY_NAMES):
    subprocess.call(
        ['../data_processing/make_all_results_zhang.sh',study,ALL_RESULTS_FILES[i],ALPHA,SNPSET])
    subprocess.call(
        ['../data_processing/make_all_betas_zhang.sh',study,ALL_BETAS_FILES[i],ALPHA,SNPSET])
    subprocess.call(['../data_processing/make_all_logs_zhang.sh',study,ALL_LOGS_FILES[i]])
    subprocess.call(
        ['../data_processing/make_all_covariances_zhang.sh',study,ALL_COVARIANCES_FILES[i],ALPHA,SNPSET])
